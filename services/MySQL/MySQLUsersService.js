    ...
    async createUser(userDTO, password) {
        const createUserCMD = new Promise((resolve, reject) => {
            this.connection.query({
                sql: "INSERT INTO users (USERNAME, FNAME, LNAME, EMAIL, PASS, MAJOR, DEGREE) VALUES(?,?,?,?,?,?,?);",
                values:[userDTO.username, userDTO.fName, userDTO.lName, userDTO.email, password, userDTO.major, userDTO.degree]
            },
            (err, results, fields) => {
                if(err) {
                    return reject(err);
     ...
     */
     async updateUser(userDTO, user_id) {
        const updateUserCMD = new Promise((resolve, reject) => {
            this.connection.query({
                sql: "UPDATE users SET USERNAME=? , FNAME=? , LNAME=? , EMAIL=? , PASS=? , MAJOR=? , DEGREE=? WHERE USER_ID=?;",
                values:[userDTO.username, userDTO.fName, userDTO.lName, userDTO.email, userDTO.password, userDTO.major, userDTO.degree, user_id]
            },
            (err, results, fields) => {
                if(err) {
                    throw(err)
